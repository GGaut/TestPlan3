...
| TK8| Жизненный цикл команды | REST запрос → Сервис А → AMQP → Сервис В → Обработка → AMQP → Сервис А → Обновление статуса | Статус обновлен, процесс прошел все узлы без ошибок. |
